...
post '/session/new' do
  ...
  if params[:login] == '' || params[:password] == ''
    ...
    elsif Digest::SHA1.hexdigest(params[:password] + result['value']['salt'].to_s) != result['value']['password_hash']
      # User found, but wrong password
      failure = "wrong_password"
    else
      # User found, and right password
      set_cookie("user_id", result['id'])
      set_cookie("user_username", result['value']['username'])
      reset_logged_in
    end
  end
...
post '/user/new' do
  ...
  if params[:email].empty?
    ...
    db.save({:type => 'user', :username => params[:login], :salt => salt, :password_hash => password_hash})
  end
